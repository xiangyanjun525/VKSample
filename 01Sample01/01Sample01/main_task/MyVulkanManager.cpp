...
void  MyVulkanManager::create_vulkan_DepthBuffer()
{
	...
	{
		image_info.tiling = VK_IMAGE_TILING_LINEAR; //采用线性组织方式
		printf("tiling为VK_IMAGE_TILING_LINEAR！\n");
	}
	else if (depthFormatProps.optimalTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) //是否支持最有瓦片组织方式
	{
		image_info.tiling = VK_IMAGE_TILING_OPTIMAL; //采用最有瓦片组织方式
	...
	VkMemoryAllocateInfo mem_alloc = {}; //构建内存分配信息结构实例
...
void MyVulkanManager::create_render_pass()
{
	...
	VkAttachmentDescription attachments[2];  //颜色附件信息数组
